...
dbimport <- function() {
  ...
  table.current.Workload <- 
    dbGetQuery(con,'
               ...
               "Workload"
               FROM "Croatia"."RADAR_CONTROLLER.WORKLOAD"
               WHERE "Time_day" = 2')
  ...
  temp1 <- aggregate(data = table.Workload,Workload~Time+Scenario,"sum")
  temp1$Sector <- "All Sectors"
  temp2 <- aggregate(data = subset(table.Workload, Sector %in% list.sectors),Workload~Time+Scenario,"sum")
  temp2$Sector <- "All TMA"
